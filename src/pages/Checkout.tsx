...
export const Checkout = () => {
  ...
  const handlePayment = async () => {
    ...
    // Simulate payment processing with a 5 second delay
    setTimeout(async () => {
      try {
        // Create order data according to the database schema
        const selectedAddressData = addresses.find(a => a.id.toString() === selectedAddress);
        const orderData = {
          user_id: user.id,
          total_amount: cartTotal,
          payment_method: paymentMethod,
          payment_details: paymentMethod === 'UPI' ? JSON.stringify({ upiId }) : JSON.stringify({ method: 'QR' }),
          status: 'PAID',
